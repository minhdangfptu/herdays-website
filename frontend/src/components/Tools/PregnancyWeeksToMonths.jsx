...
export default function PregnancyWeeksToMonths() {
  ...
  const handleCalculate = (e) => {
    ...
    if (totalWeeks < 1 || totalWeeks > 42) {
      alert("Số tuần thai thường dao động từ 1 đến 42 tuần nha bồ!");
      return;
    }
    ...
    else if (totalWeeks >= 14 && totalWeeks <= 17) { month = 4; trimester = 2; }
    else if (totalWeeks >= 18 && totalWeeks <= 22) { month = 5; trimester = 2; }
    else if (totalWeeks >= 23 && totalWeeks <= 27) { month = 6; trimester = 2; }
    else if (totalWeeks >= 28 && totalWeeks <= 31) { month = 7; trimester = 3; }
    else if (totalWeeks >= 32 && totalWeeks <= 35) { month = 8; trimester = 3; }
    else if (totalWeeks >= 36) { month = 9; trimester = 3; }
  ...
  return (
    <div className="calculator-widget">
      ...
      <p className="calc-desc">Quy đổi siêu tốc tuổi thai khoa học sang số tháng dân dã.</p>
      ...
      <form onSubmit={handleCalculate}>
        ...
        <button type="submit" className="calc-btn">Quy đổi ngay</button>
      </form>

      <div className="calc-note">
        <span className="note-icon">💡</span>
        <p>
          <strong>Lưu ý:</strong> Một tháng thai kỳ thường dài hơn 4 tuần một chút (khoảng 4.3 tuần). Do đó, cách chia tháng này được tính theo các mốc phát triển sinh lý của em bé.
        </p>
      </div>
