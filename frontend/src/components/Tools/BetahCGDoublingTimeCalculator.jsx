...
export default function BetahCGDoublingTimeCalculator() {
  ...
  const handleCalculate = (e) => {
    ...
    if (val2 <= val1) {
      alert("Nồng độ Beta hCG lần 2 phải lớn hơn lần 1.");
      return;
    }
    if (time2 <= time1) {
      alert("Thời điểm xét nghiệm lần 2 phải diễn ra sau lần 1.");
      return;
    }
  ...
  return (
    <div className="calculator-widget">
      ...
      <p className="calc-desc">Theo dõi sự phát triển của thai kỳ giai đoạn đầu thông qua chỉ số máu.</p>
      
      <form onSubmit={handleCalculate}>
        <div className="form-group">
          <label>Nồng độ Beta hCG lần 1 (mIU/mL)</label>
          <input type="number" className="form-input" value={hcg1} onChange={(e) => setHcg1(e.target.value)} required />
        </div>
        <div className="form-group">
          <label>Thời điểm xét nghiệm lần 1</label>
          <input type="datetime-local" className="form-input" value={date1} onChange={(e) => setDate1(e.target.value)} required />
        </div>
        <div className="form-group">
          <label>Nồng độ Beta hCG lần 2 (mIU/mL)</label>
          <input type="number" className="form-input" value={hcg2} onChange={(e) => setHcg2(e.target.value)} required />
        </div>
        <div className="form-group">
          <label>Thời điểm xét nghiệm lần 2</label>
          <input type="datetime-local" className="form-input" value={date2} onChange={(e) => setDate2(e.target.value)} required />
        </div>

        <button type="submit" className="calc-btn">Phân tích chỉ số</button>
      </form>
      ...
      {result && (
        <div className="result-box">
          ...
          <div className="result-item">
            ...
            <span className="value">{result.hours} giờ ({result.days} ngày)</span>
          </div>
          <div className="result-item">
