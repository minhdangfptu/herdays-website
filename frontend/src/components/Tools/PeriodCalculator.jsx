...
export default function PeriodCalculator() {
  ...
  const handleCalculate = (e) => {
    ...
    const nextStart = new Date(startDate.getTime() + cycleLength * 24 * 60 * 60 * 1000);
  ...
  return (
    <div className="calculator-widget">
      ...
      <p className="calc-desc">Chuẩn bị sẵn sàng "vũ khí" cho ngày rụng dâu tiếp theo của bạn.</p>
      ...
      <form onSubmit={handleCalculate}>
        ...
        <div className="form-group">
          ...
          <input type="number" className="form-input" value={periodDuration} onChange={(e) => setPeriodDuration(Number(e.target.value))} min="2" max="10" required />
        </div>

        <button type="submit" className="calc-btn">Dự đoán ngay</button>
      </form>

      <div className="calc-note">
        <span className="note-icon">💡</span>
        <p>
          <strong>Lưu ý:</strong> Căng thẳng, chế độ ăn uống và giấc ngủ có thể làm chu kỳ thực tế xê dịch từ 1-3 ngày so với dự đoán máy tính.
        </p>
      </div>
