...
function AdminProductsPage() {
  ...
  const productModalTitle = editingProduct ? 'Cập nhật sản phẩm' : 'Tạo mới sản phẩm';
  ...
  const handleSaveProduct = async (event) => {
    ...
    try {
      const thumbnail = await uploadThumbnailIfNeeded(productImageFile, productForm.thumbnail, 'product');
      if (!thumbnail) {
  ...
  const handleSaveBox = async (event) => {
    ...
    try {
      const thumbnail = await uploadThumbnailIfNeeded(boxImageFile, boxForm.thumbnail, 'box');
      if (!thumbnail) {
        ...
        <div className="mb-3 flex flex-col gap-3 rounded-lg bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              placeholder={isBoxTab ? 'Tìm kiếm box...' : 'Tìm kiếm sản phẩm...'}
            />
          </div>
          ...
          <table className="w-full min-w-[960px] border-collapse text-left">
            <thead className="bg-white text-xs font-bold uppercase tracking-wide text-slate-400">
              <tr className="border-b border-slate-100">
                ...
                <th className="px-5 py-4">{isBoxTab ? 'Tên box' : 'Tên sản phẩm'}</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr>
                  <td className="px-5 py-10 text-center font-semibold text-slate-400" colSpan={8}>Đang tải dữ liệu...</td>
                </tr>
              ) : errorMessage ? (
                <tr>
                ...
                <tr>
                  <td className="px-5 py-10 text-center font-semibold text-slate-400" colSpan={8}>
                    Không có dữ liệu phù hợp.
                  </td>
                </tr>
              ) : rows.map((item, index) => (
                <tr key={item.id} className="transition hover:bg-pink-50/30">
                  <td className="px-5 py-4 text-sm font-bold text-slate-700">{(page - 1) * PAGE_SIZE + index + 1}</td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      {item.thumbnail ? (
                        <img className="h-10 w-10 rounded-md object-cover" src={item.thumbnail} alt={item.productName} />
                      ) : (
                        <div className="flex h-10 w-10 items-center justify-center rounded-md bg-pink-50 text-pink-400">
                          {isBoxTab ? <Box size={18} /> : <PackageSearch size={18} />}
                        </div>
                      )}
                      <div>
                        ...
                        {isBoxTab && (
                          <p className="mt-1 line-clamp-1 text-xs font-medium text-slate-400">
                            {(item.products || []).map((product) => product.productName).filter(Boolean).join(', ') || 'Chưa chọn sản phẩm'}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <td className="px-5 py-4 text-sm font-semibold text-slate-800">{formatCurrency(item.price)}</td>
                  <td className="px-5 py-4 text-sm text-slate-600">{item.quantity}</td>
                  <td className="px-5 py-4">
                    ...
                    >
                      {Number(item.quantity) > 0 ? 'Đang hoạt động' : 'Ngừng theo dõi'}
                    </span>
                  </td>
      ...
      {isProductModalOpen && (
        <AdminModal title={productModalTitle} onClose={closeProductModal}>
          <form onSubmit={handleSaveProduct} className="space-y-4">
            <FormInput label="Tên sản phẩm" required value={productForm.productName} onChange={(value) => handleProductFieldChange('productName', value)} />
            <ImageUploadField
            ...
            <FormTextarea label="Ghi chú" value={productForm.description} onChange={(value) => handleProductFieldChange('description', value)} />
            <ModalActions isSubmitting={isSubmitting} onCancel={closeProductModal} submitLabel={editingProduct ? 'Cập nhật' : 'Xác nhận'} />
          </form>
        </AdminModal>
      )}
      ...
      {isBoxModalOpen && (
        <AdminModal title={boxModalTitle} onClose={closeBoxModal}>
          <form onSubmit={handleSaveBox} className="space-y-4">
            ...
            <ModalActions isSubmitting={isSubmitting} onCancel={closeBoxModal} submitLabel={editingBox ? 'Cập nhật' : 'Xác nhận'} />
          </form>
        </AdminModal>
      )}
    </main>
...
function ImageUploadField({ label, currentUrl, file, onChange, required = false }) {
  return (
    <label className="block">
      <span className="mb-1 block text-sm font-bold text-slate-700">
        {label}{required && <span className="text-pink-500"> *</span>}
      ...
      <div className="flex items-center gap-3 rounded-lg border border-slate-200 p-3">
        {currentUrl ? (
          <img className="h-14 w-14 rounded-md object-cover" src={currentUrl} alt={label} />
        ) : (
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 truncate text-xs font-medium text-slate-400">
            {file ? file.name : 'Chọn ảnh từ máy local, hệ thống sẽ upload lên Cloudinary khi lưu.'}
          </p>
        </div>
      </div>
    </label>
